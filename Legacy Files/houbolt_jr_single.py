def houbolt_jr_single():
    ...
    # Masses
    input.mass.dry              = input.mass.data.Dry_Mass
    ...
    input.fPres.mTank           = input.mass.data.Fuel_Pressurant_Tank
    ...
    input.fPres.Rhoinit         = py.CoolProp.CoolProp.PropsSI('D', 'T', input.fPres.Tinit, 'P', input.fPres.Pinit, input.fPres.name)
    ...
    input.fuel.mTank            = input.mass.data.Fuel_Tank
    input.fuel.lTank            = 0.27                 # READ FROM MASS MUDGET         [m]
    input.fuel.ullage           = 0.05
    input.fuel.mInit            = input.mass.data.Fuel_Mass   # Initial mass           [m]
    input.fuel.Tinit            = 298
    input.fuel.Pinit            = 525*input.settings.cnv
    input.fuel.Rhoinit          = py.CoolProp.CoolProp.PropsSI('D', 'T', input.fuel.Tinit, 'P', input.fuel.Pinit, 'Ethanol')
    input.fuel.vTank            = input.fuel.mInit * (1 + input.fuel.ullage) / input.fuel.Rhoinit             
    input.fuel.lTank            = input.fuel.vTank / (pi*(0.5*input.design.diameter - input.fuel.tTank)^2)    
    ...
    input.oxPres.Rhoinit        = py.CoolProp.CoolProp.PropsSI('D', 'T', input.oxPres.Tinit, 'P', input.oxPres.Pinit, input.oxPres.name)
    ...
    input.ox.mInit              = input.mass.data.Ox_Mass
    input.ox.Tinit              = 278
    input.ox.Pinit              = 525*input.settings.cnv
    input.ox.Rhoinit            = py.CoolProp.CoolProp.PropsSI('D', 'T', input.ox.Tinit, 'P', input.ox.Pinit, input.ox.name)
    input.ox.ullage             = 0.05
    input.ox.tTank              = 0.003175
    input.ox.mTank              = input.mass.data.Ox_Tank
    input.ox.vTank              = input.ox.mInit * (1 + input.ox.ullage) / input.ox.Rhoinit             
    input.ox.lTank              = input.ox.vTank / (pi*(0.5*input.design.diameter - input.ox.tTank)^2)                  
    input.ox.order              = 2                    # Order inside rocket           [integer]
    input.ox.offset             = 0.31                 # Distance till next comp.      [m]
    
    input.props = cellss(4,3)
    input.props[1][1] = 'Pressurant'
    input.props[1][2] = input.fPres
    input.props[1][3] = 0
    input.props[2][1] = 'Fuel'
    input.props[2][2] = input.fuel
    input.props[2][3] = 1
    input.props[3][1] = 'Pressurant'
    input.props[3][2] = input.oxPres
    input.props[3][3] = 0
    input.props[4][1] = 'Oxidizer'
    input.props[4][2] = input.ox
    input.props[4][3] = 3
