...
def _custom_plot(x_data, datas_dict, x_label, x_unit, y_label, y_unit, title, style, exclusion, save):
...
def _plot_generalinfo(flight_run, style, exclusion, save):
    ...
    if save:
        plt.savefig("output/General Info.png")
        plt.clf()
    else:
        plt.show()

def _plot_position(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Position', '(m)', "Position vs. Time Plot", style, exclusion, save)

def _plot_velocity(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Velocity', '(m/s)', "Velocity vs. Time Plot", style, exclusion, save)

def _plot_acceleration(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Acceleration', '(m/s2)', "Acceleration vs. Time Plot", style, exclusion, save)

def _plot_quaternion(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Quaternion', '', "Quaternion vs. Time Plot", style, exclusion, save)

def _plot_angle(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Angle', '(rad)', "Euler Angle vs. Time Plot", style, exclusion, save)

def _plot_omega(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Angular Velocity', '(rad/s)', "Angular Velocity vs. Time Plot", style, exclusion, save)

def _plot_alpha(flight_run, style, exclusion, save):
    t = flight_run.state_vector[0]
    datas_dict = {
    ...
    _custom_plot(t, datas_dict, 'Time', '(s)', 'Angular Acceleration', '(rad/s2)', "Angular Acceleration vs. Time Plot", style, exclusion, save)

def _plot_path(flight_run, style, exclusion, save):
...
def _plot_animation(flight_run, style, exclusion, save):
    if save:
        print("Animation unable to save to png")
    else:
...
def plot(flight_run, arg="All", style="Combined", exclusion=[], save=False):
    ...
    if "all" in arg or "generalinfo" in arg:
        _plot_generalinfo(flight_run, style, exclusion, save)
    if "all" in arg or "position" in arg:
        _plot_position(flight_run, style, exclusion, save)
    if "all" in arg or "velocity" in arg:
        _plot_velocity(flight_run, style, exclusion, save)
    if "all" in arg or "acceleration" in arg:
        _plot_acceleration(flight_run, style, exclusion, save)
    if "all" in arg or "quaternion" in arg:
        _plot_quaternion(flight_run, style, exclusion, save)
    if "all" in arg or "angle" in arg or "euler angle" in arg:
        _plot_angle(flight_run, style, exclusion, save)
    if "all" in arg or "omega" in arg:
        _plot_omega(flight_run, style, exclusion, save)
    if "all" in arg or "alpha" in arg:
        _plot_alpha(flight_run, style, exclusion, save)
    if "all" in arg or "path" in arg:
        _plot_path(flight_run, style, exclusion, save)
    if "all" in arg or "3danimation" in arg:
        _plot_animation(flight_run, style, exclusion, save)
