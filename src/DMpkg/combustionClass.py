...
class combustionClass:
    ...
    def get_CEA(self):
        ...
        houbolt_jr                     = rocket.cea(Pcc=self.input["design"]["Pcc"], OF=self.input["design"]["OF"], Pamb= 14.7,  oxName= self.input["ox"]["name"], fuelName= self.input["fuel"]["name"]) #are these meant to be default values? where to get params from?
